...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune torchvision ResNet50.")
    parser.add_argument("--data", default="CNN_dataset4")
    parser.add_argument("--output", default="runs/resnet50_5")
...
def build_transforms(image_size, augment):
    ...
    eval_transform = v2.Compose(
        [v2.ToImage(),
         ...
         v2.ToDtype(torch.float32, scale=True), normalize]
    )
    if not augment:
...
def main():
    ...
    best_epoch = 0
    history = []
    checkpoint_path = output_dir / "best_resnet50.pt"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            best_weights = copy.deepcopy(model.state_dict())
            torch.save(
                {"architecture": "resnet50", "model_state": best_weights, "class_to_idx": class_to_idx,
    ...
    metrics = {"architecture": "resnet50", "best_epoch": best_epoch, "best_val_loss": best_val_loss,
