...
class ShrimpSexRatioAnalyzer:
    ...
    def run(
        ...
        with tqdm(total=sample_count, desc="Analyze frames", unit="frame", ncols=85) as pbar:
            while live_source or frame_idx < total_frames:
                ...
                if grayscale:
                    frame = self._to_grayscale_bgr(frame)
        ...
        summary["HBB_Temporal_Enabled"] = False
    ...
    def _detect_frame(
        ...
        hbb_inputs = [meta["crop"] for meta in metas]
        detections = []
        ...
        for meta, res in zip(metas, hbb_results):
            ...
            cache_key = int(track_id) if track_id is not None else None
            if male_line_box_crop is not None:
                if cache_key is not None:
                    self._hbb_box_cache[cache_key] = self._normalize_crop_box(
                        male_line_box_crop,
                        meta["crop"].shape[1],
                        meta["crop"].shape[0],
                    )
            elif cache_key is not None and cache_key in self._hbb_box_cache:
                male_line_box_crop = self._denormalize_crop_box(
                    self._hbb_box_cache[cache_key],
                    meta["crop"].shape[1],
                    meta["crop"].shape[0],
                )
                male_line_pts = self._project_crop_box(male_line_box_crop, meta["inverse_matrix"])
                hbb_box_source = "cached"

            cnn_is_male = False
            cnn_male_conf = 0.0
            if male_line_box_crop is not None:
                male_line_crop = MaleLineCNNClassifier.crop_hbb_box(meta["crop"], male_line_box_crop)
                cnn_is_male, cnn_male_conf = self.cnn_classifier.predict_crop(male_line_crop)
                if not cnn_is_male:
                    male_line_pts = None
            ...
            detections.append({
                **meta,
                "is_male": cnn_is_male,
                "male_conf": cnn_male_conf,
                "hbb_conf": hbb_conf,
                "hbb_box_source": hbb_box_source,
    ...
    def _project_hbb_box(box, inverse_matrix) -> np.ndarray:
        ...
        return ShrimpSexRatioAnalyzer._project_crop_box([x1, y1, x2, y2], inverse_matrix)

    @staticmethod
    def _project_crop_box(box_xyxy, inverse_matrix) -> np.ndarray:
    ...
    @staticmethod
    def _normalize_crop_box(box_xyxy, crop_width: int, crop_height: int) -> list[float]:
        x1, y1, x2, y2 = [float(v) for v in box_xyxy]
        crop_width = max(int(crop_width), 1)
        crop_height = max(int(crop_height), 1)
        return [
            max(0.0, min(x1 / crop_width, 1.0)),
            max(0.0, min(y1 / crop_height, 1.0)),
            max(0.0, min(x2 / crop_width, 1.0)),
            max(0.0, min(y2 / crop_height, 1.0)),
        ]

    @staticmethod
    def _denormalize_crop_box(norm_box, crop_width: int, crop_height: int) -> list[float]:
        x1, y1, x2, y2 = [float(v) for v in norm_box]
        crop_width = max(int(crop_width), 1)
        crop_height = max(int(crop_height), 1)
        px1 = max(0.0, min(x1 * crop_width, crop_width - 1.0))
        py1 = max(0.0, min(y1 * crop_height, crop_height - 1.0))
        px2 = max(px1 + 1.0, min(x2 * crop_width, float(crop_width)))
        py2 = max(py1 + 1.0, min(y2 * crop_height, float(crop_height)))
    ...
    def _record_detection(det: dict, frame_idx: int, fps: float) -> dict:
        ...
        return {
            ...
            "HBB_Conf": round(float(det.get("hbb_conf", 0.0)), 4),
            "HBB_Box_Source": det.get("hbb_box_source", ""),
            "CNN_Male_Prob": round(float(det.get("male_conf", 0.0)), 4),
    ...
    def _crop_preview_tile(det: dict, width: int, height: int, idx: int):
        ...
        preview_box = det.get("male_line_box_crop") if det.get("is_male") else None
        draw_crop, male_line_box = ShrimpSexRatioAnalyzer._trim_preview_crop(crop, preview_box)
